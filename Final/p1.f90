...
implicit none
  ...
  integer, parameter              :: nl=8, N=512, t_eq=2500, t_scal=125, t_run=2500, nsim=1
  real(pr)                        :: T
  real(pr), dimension(1:5000)     ::  Ti,   Ec,  Ep,  Et,  P
...
DO ns = 1, nsim          ! Promediamos sobre nsim simulaciones independientes
  ...
  call SC(x)       ! Acomodamos las particulas segùn una SC
...
! Normalizamos los valores instantaneos y los anotamos
  ...
  open(511,file='1a.d')
  do i = 1, t_eq+t_run
    write(511,*) i*dt, Ti(i), Ec(i), Ep(i), Et(i), P(i)
  end do
  close(511)

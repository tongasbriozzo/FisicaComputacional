...
implicit none
  ...
  integer                           :: i, j, np, ns
  integer, parameter                :: nl=8, N=512, t_eq=1000, t_scal=50, t_run=10000, nsim=1000, nr=500, nh=10000
  real(pr)                          :: T, Ti
  ...
  real(pr), parameter               :: rho = 1.2_pr, Vol=real(N,pr)/rho, L=(Vol)**(1._pr/3._pr), a=L/((N/4._pr)**(1._pr/3._pr))
...
! Inicializamos el Parametro de Orden Cristalino
  ...
  open(522,file='poc1.2.d')  ! Aqui anotaremos las medidas del sistema en funcion de los pasos evolucionados
  do ns = 1, nsim         ! Promediaremos sobre nsim simulaciones
    write(*,*) 'nsim poc', ns
    call SC(x)           ! Acomodamos las particulas segùn una FCC
...
! Normalizamos y escribimos los resultados
  S(:) = S(:)/real(nsim,pr)
  do i = 1, t_eq
    write(522,*) real(i,pr)*dt, S(i)                 ! Anotamos los valores obtenidos
  end do
  close(522)
...
!*******************************************************************************
  ...
  implicit none
    ...
    sum_i = 0._pr
    do ii = 1, N
      kr = 16._pr*pi*(xx(ii,1)+xx(ii,2)+xx(ii,3))/L
      sum_r = sum_r + cos(kr)
      sum_i = sum_i + sin(kr)
    end do

...
! de contorno periodicas, en funcion de la temperatura.
! ns debe cambiarse manualmente para probar distintos sistemas
!
...
implicit none
  ...
  integer                       :: i, j, k
  integer                       :: nf, nc
  integer, parameter            :: ns=10, np=5, nsim=50, ndes=10000000
! Considere que correr el programa con ns=40 lleva aproximadamente 24 hs
  integer, dimension(1:ns,1:ns) :: s
  real(pr)                      :: V, T, E, M, C, X, H2, mu2, xt
!  real(pr), parameter           :: at=12.55592670560186_pr, bt=-21.29665330186477, ct=12.04072659926291

! Definimos los formatos que emplearemos en los documentos de texto
  1 format(A11,   14x, A7,    18x, A16,    9x, A13,   12x, A15)
  2 format(E23.16, 2x, E23.16, 2x, E23.16, 2x, E23.16, 2x, E23.16)
...
! Abrimos el documento de dexto y describimos los datos a anotar
  open(420,file='10.d')
  write(420,1) 'Temperatura', 'Energia', 'Calor Especifico', 'Magnetizacion', 'Susceptibilidad'

! Calculamos E, M, C y X para cada T, promediando sobre nsim simulaciones
  do i = 0, 10
!   Definimos la temperatura
!   Compruebe que ésta es una distribucion de probabilidades que concentra
!   puntos cerca de la temperatura crítica 2.2692, vale 0 en i=0 y 3.3 en i=500
!    xt = 0.002_pr*real(i,pr)
!    T  = at*xt**3 + bt*xt**2 + ct*xt
    T = 0.06_pr*real(i,pr)
!   Comprobamos la evolucion del programa
    write(*,*) 'Temperatura', i, xt, T
!   Inicializamos las mediciones
    E = 0._pr
    M = 0._pr
    C = 0._pr
    X = 0._pr
...
!     Calculamos el cuadrado de valores de expectacion y valores de expectacion del cuadrado
      E   = E   +  Ham(s)        ! <H>
      H2  = H2  + (Ham(s))**2    ! <H^2>
      M   = M   + abs(mu(s))     ! <mu>
      mu2 = mu2 +    (mu(s))**2  ! <mu^2>
...
!   Normalizamos los valores obtenidos
    E   = E/real(nsim,pr)/V        ! Energia por unidad de volumen
    H2  = H2/real(nsim,pr)/(V*V)   ! Hamiltoniano al cuadrado
    M   = M/real(nsim,pr)/V        ! Magnetizacion
    mu2 = mu2/real(nsim,pr)/(V*V)  ! Magnetizacion al cuadrado
    C = (H2  - E**2)*V/(T*T)       ! Calor especifico
    X = (mu2 - M**2)*V/T           ! Susceptibilidad magnetica
!   Anotamos los valores obtenidos
    write(420,2) T, E, C, M, X
  end do
